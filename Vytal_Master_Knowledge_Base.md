...
## 4. MASTER ALGORITHM & RESEARCH REGISTRY (GROUPS A–M)

### Section 0. Repository Access & Infrastructure `[SELF-CHECKABLE]`
- **Item 0: Public Repository Access (`github.com/Ahmad-Ali-Shah/Vital`)** `[VERIFY]`
...
- **Item 1: CHROM Chrominance-Based rPPG Extraction** `[VERIFY]`
  - *Research:* de Haan, G. & Jeanne, V. (2013). "Robust Pulse Rate From Chrominance-Based rPPG." *IEEE Trans Biomed Eng*, 60(10), 2878–2886. PubMed: [23744659](https://pubmed.ncbi.nlm.nih.gov/23744659/).
...
- **Item 2: Goertzel Single-Bin Frequency Detection** `[VERIFY]`
  - *Research:* Goertzel, G. (1958). *Amer Math Monthly*, 65(1), 34–35.
  - *Task:* Ensure `rppg.js` constrains search window strictly to 0.7 Hz – 3.0 Hz (42–180 BPM) to prevent locking onto 50/60 Hz ambient lighting flicker.
  - *Owner:* Backend Engineer (`b0c1f3854139`).
- **Item 3: MediaPipe Task Choice for Face ROI Tracking** `[VERIFY]`
  - *Research:* Google AI Edge — MediaPipe Face Landmarker (478 3D points).
  - *Task:* Verify `ScanPage.jsx` uses `FaceLandmarker` rather than 2D `FaceDetector` to prevent ROI drift during micro-movements.
  - *Owner:* Frontend Engineer (`cff36e25cd5e`) & Backend Engineer (`b0c1f3854139`).
- **Item 4: Green-Channel Dominance in rPPG** `[VERIFY]`
  - *Research:* Verkruysse, W. et al. (2008). *Optics Express*, 16(26), 21434–21445. PubMed: [19104573](https://pubmed.ncbi.nlm.nih.gov/19104573/).
...
- **Item 5: Ratio-of-Ratios (RoR) Channel Selection for SpO2** `[SELF-CHECKABLE + VERIFY]`
  - *Research:* Ding, X. et al. (2023). *Sensors*, 23(2), 737. PMC: [PMC9863359](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC9863359/).
  - *Task:* Verify `spo2.js`: confirm whether Red/Green or Red/Blue ratio is implemented. Align code comments and research dossier.
  - *Owner:* Backend Engineer (`b0c1f3854139`) & Data Analyst (`cf0c0821d3e7`).
- **Item 6: RoR → SpO2% Empirical Calibration Curve** `[VERIFY]`
  - *Task:* Trace calibration coefficients in `spo2.js` to empirical clinical trial data in the research dossier ($SpO_2 = 110 - 25 \cdot R$).
  - *Owner:* Backend Engineer (`b0c1f3854139`) & Data Analyst (`cf0c0821d3e7`).
...
- **Item 14: Scleral Icterus → Bilirubin Estimation** `[VERIFY]`
  ...
  - *Task:* Confirm ambient light subtraction in `jaundice.js` prior to chromaticity calculation.
  - *Owner:* Backend Engineer (`b0c1f3854139`).

### Group C — Arrhythmia & HRV Analytics (`afib.js`)
- **Item 7: RMSSD & pNN50 Threshold Sourcing** `[VERIFY]`
  - *Research:* ESC/NASPE Task Force (1996). *Circulation*, 93(5), 1043–1065. PMC: [PMC12870696](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC12870696/).
...
- **Item 9: 3-Level Header vs 4-Tier Palette Reconciliation** `[SELF-CHECKABLE]`
  - *Task:* Align README, UI components, and clinical dossier to explicit 4-tier WHO palette (GREEN, YELLOW, ORANGE, RED).
...
- **Item 10: WHO IMCI Color-Scale Mapping** `[VERIFY]`
  - *Research:* WHO IMCI Guidelines. *JOGH* 13:03024 (2023).
...
- **Item 11: PALS Pediatric Reference Ranges** `[VERIFY]`
  - *Research:* StatPearls: [NBK553213](https://www.ncbi.nlm.nih.gov/books/NBK553213/). PMC: [PMC7465456](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7465456/).
...
- **Item 12: Maternal Baseline Adjustments** `[VERIFY]`
  - *Task:* Confirm 3rd trimester obstetric baseline adjustment factors in `alertScale.js`.
  - *Owner:* Data Analyst (`cf0c0821d3e7`) & Product Manager (`e2f045dbc0a1`).
...
- **Item 15: Pulse Transit Time (PTT) vs Pulse Wave Analysis (PWA)** `[SCOPE + VERIFY]`
  - *Research:* *Sci Rep* (2017). PMC: [PMC5599606](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5599606/).
...
- **Item 16: Mid-Upper Arm Circumference (MUAC) vs Camera BMI for SAM Screening** `[VERIFY]`
  - *Research:* WHO Malnutrition Guidelines (2023). [NBK601660](https://www.ncbi.nlm.nih.gov/books/NBK601660/).
  - *Task:* Combine camera shoulder ratio with reference card scaling to estimate MUAC equivalent (<115 mm).
...
- **Item 18: Shewhart SPC & EWMA Small-Sample Control Limits** `[VERIFY]`
  - *Research:* Springer (2021). [10.1007/978-3-030-67856-2_2](https://link.springer.com/chapter/10.1007/978-3-030-67856-2_2).
...
- **Item 19: FHIR R4 Observation Required Fields** `[VERIFY]`
  - *Research:* HL7 FHIR R4 Specification. [vitalsigns profile](https://hl7.org/fhir/R4/observation-vitalsigns.html).
  - *Task:* Validate exported JSON against FHIR R4 schema in `platform.js`.
  - *Owner:* QA Engineer (`06bae13804b8`) & Backend Engineer (`b0c1f3854139`).
- **Item 20: DHIS2 Tracker Program vs Event Program** `[VERIFY]`
  - *Research:* DHIS2 Tracker Implementation Guide.
  - *Task:* Configure `platform.js` to emit DHIS2 Tracker Program payloads for patient history continuity.
  - *Owner:* QA Engineer (`06bae13804b8`) & Backend Engineer (`b0c1f3854139`).

### Group H — Device & Platform API Integrations
- **Item 21: Hardware Integration Labeling** `[SCOPE]` (Product Manager `e2f045dbc0a1` & DevOps `f37c976fa739`).
- **Item 22: LLM AI Explanation Authority Boundary** `[VERIFY]` (Product Manager `e2f045dbc0a1` & Backend `b0c1f3854139`).
- **Item 23: Offline Storage Reliability** `[SCOPE]` (DevOps Engineer `f37c976fa739`).
- **Item 24: Platform Sharing Triggers** `[SCOPE]` (Product Manager `e2f045dbc0a1`).

### Group I — Workflow Governance & Swarm Principles
- **Item 25: Stoudt ERP 3-Phase Workflow Principles** `[VERIFY]` (Plos Comp Bio 2021 PMC7971542).
- **Item 26: van der Aalst ECAA Workflow Rules & Closed Loops** `[VERIFY]` (Process Mining 2016).
