...
export function analyzeConjunctivalPallor(ctx, roi) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    // desaturated/neutral like shadow, hair, or background clutter)
    const brightness = (r + g + b) / 3
    if (brightness < 25 || brightness > 250) continue

    const { s } = rgbToHsv(r, g, b)
    if (s < 0.08) continue // skin/conjunctival tissue threshold

    // Detect cyanotic hypoxemic shift (Blue channel dominating Red channel)
    if (b > r * 1.2) {
      cyanosisPixelCount++
    }
  ...
  // Minimum valid pixel count calibrated for eye ROI crops at standard camera distances
  const MIN_VALID_PIXELS = 15
  ...
  const isCyanotic = totalValidPixels > 0 && (cyanosisPixelCount / totalValidPixels) >= 0.40
  ...
  if (isCyanotic) {
    ...
    recommendation = 'EMERGENCY REFERRAL: Mucosal cyanotic shift detected (SpO2 hypoxia proxy). Immediate clinical evaluation required.'
  } else if (clampedHb < 7.0) {
    tier = 'RED'
