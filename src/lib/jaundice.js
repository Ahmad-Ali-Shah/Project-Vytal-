...
function estimateGrayWorldCorrection(ctx) {
  try {
    ...
    // yellow or red sclera) can't wildly over-correct away true jaundice signals.
    // If Red and Green dominate over Blue (hallmark of scleral yellowing/icterus),
    // clamp bGain to prevent artificial blue boosting.
    const isYellowDominant = rAvg > bAvg * 1.15 && gAvg > bAvg * 1.15
    const clampGain = (g) => Math.max(0.6, Math.min(1.6, g))
    const maxBGain = isYellowDominant ? 1.10 : 1.6

    return {
      rGain: clampGain(grayTarget / rAvg),
      gGain: clampGain(grayTarget / gAvg),
      bGain: Math.max(0.6, Math.min(maxBGain, grayTarget / bAvg)),
    }
  } catch (e) {
...
export function analyzeScleralIcterus(ctx, roi) {
  ...
  // If the ROI spans nearly the entire canvas (meaning it is a pre-cropped ROI image),
  // skip gray-world re-balancing to prevent washing out genuine sclera chromaticity into neutral gray.
  const isIsolatedCrop = (roi.w >= (ctx.canvas.width || 1) * 0.8) && (roi.h >= (ctx.canvas.height || 1) * 0.8)
  const { rGain, gGain, bGain } = isIsolatedCrop
    ? { rGain: 1, gGain: 1, bGain: 1 }
    : estimateGrayWorldCorrection(ctx)
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    // Reject non-sclera dark pixels
    const brightness = (r + g + b) / 3
    if (brightness < 40) continue

    const { h, s } = rgbToHsv(r, g, b)

    // Reject intense red subconjunctival hemorrhage pixels (h < 18° or h > 340° with high saturation)
    // to prevent blood spots from corrupting scleral yellowing ratio calculation.
    if ((h < 18 || h > 340) && s >= 0.40) continue

    // Yellow hue band: 35° to 70° in HSV space
    if (h >= 35 && h <= 70 && s >= 0.15) {
      yellowPixelCount++
    }
    totalScleraPixels++
  ...
  const isJaundiced = yellowIndex >= 18
